...
class fifo_smoke_test extends fifo_base_test;
  ...
  function new(string n, uvm_component p); super.new(n,p); endfunction
  task run_phase(uvm_phase phase);
    fifo_smoke_vseq vseq = fifo_smoke_vseq::type_id::create("vseq");
    phase.raise_objection(this); vseq.start(vseqr); phase.drop_objection(this);
  endtask
endclass
...
class fifo_stress_test extends fifo_base_test;
  ...
  task run_phase(uvm_phase phase);
    fifo_stress_vseq vseq = fifo_stress_vseq::type_id::create("vseq");
    phase.raise_objection(this);
    cfg.num_ops = 5000; vseq.start(vseqr);
    phase.drop_objection(this);
  endtask
endclass
...
class fifo_reset_stress_test extends fifo_base_test;
  ...
  task run_phase(uvm_phase phase);
    fifo_stress_vseq vseq = fifo_stress_vseq::type_id::create("vseq");
    phase.raise_objection(this);
    cfg.num_ops = 8000; vseq.start(vseqr);
    phase.drop_objection(this);
  endtask
endclass
endpackage